...
test.describe('Folder Features', () => {
  ...
  test.beforeEach(async ({ authenticatedPage }) => {
    ...
    // Setup logged-in state in storage for Angular Guard
  ...
  test('should open share prompt for folder and call share API', async ({ authenticatedPage }) => {
    ...
    // Click Share Directory - ensure we wait for it
    // The menu item might be inside or outside the component depending on implementation
    const shareBtn = page.locator('button:has-text("SHARE_DIRECTORY"), [role="menuitem"]:has-text("SHARE_DIRECTORY")').first();
